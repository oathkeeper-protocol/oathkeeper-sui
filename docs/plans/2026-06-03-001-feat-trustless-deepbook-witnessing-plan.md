...
**2026-06-12 integration update:** Research and a near-runnable TypeScript harness landed. Current DeepBook testnet package is version 19 at `0x74cd5657843c627f3d80f713b71e9f895bbbeb470956d8a8e1185badf6cc77c8`; `0x22be4c...` still exists but is version 17, and shared object types retain the original package id `0xfb28c4cb...`. The current BalanceManager-anchored witnessed path requires DEEP fee funding; the no-DEEP/input-fee route applies only to direct non-manager swaps. The harness fails before gas on local `sui 1.60.0-homebrew`, so U1 remains blocked until the human runs `brew upgrade sui`.
...
- [ ] **Unit 4: `trade_via_deepbook` witnessed-execution entry (IN PROGRESS; source promoted, build green, tests still toolchain-blocked)** *(Module promoted to `contracts/sources/witnessed.move`: `mint_witnessed` reads `balance()` for the anchor; `trade_via_deepbook` swaps + reads post-balance + calls `oath::record_equity_update`; no caller-supplied numbers. WITNESSED settlement now goes through `settle_epoch_witnessed`, which reads a final BalanceManager equity anchor before the shared settlement math; the old `oath::settle_epoch` rejects WITNESSED oaths so callers cannot skip the final anchor. `oathkeeper::registry` was renamed to `oathkeeper::oath_registry` to remove the DeepBook `registry` module collision under Sui CLI 1.60. `sui move build` is green; `sui move test` still fails before Oathkeeper tests because DeepBook dependency tests require newer `std::unit_test::destroy`, so the human-gated `brew upgrade sui` remains required before marking this unit complete.*
