...
export interface Oath {
  ...
  /** True when this oath was read from the live testnet deployment (not mock). */
  onchain?: boolean;
...
// ─── On-chain snapshot (real testnet oaths) ─────────────────────────────────
// Generated by agent/src/snapshot.ts reading the live deployment. When present and
// non-empty, the marketplace renders these REAL oaths; otherwise it falls back to the
// MOCK_OATHS below. Regenerate after seeding: cd agent && bunx tsx src/snapshot.ts
import onchainSnapshot from "./onchain-snapshot.json";

// ─── Oaths (mock fallback) ───────────────────────────────────────────────────

const MOCK_OATHS: Oath[] = [
...
// ─── Live-or-mock selection ──────────────────────────────────────────────────
// Prefer real testnet oaths from the snapshot; fall back to MOCK_OATHS if the
// snapshot is empty (e.g. before the package is deployed/seeded).
const SNAPSHOT_OATHS = (onchainSnapshot.oaths ?? []) as unknown as Oath[];

export const OATHS: Oath[] = SNAPSHOT_OATHS.length > 0 ? SNAPSHOT_OATHS : MOCK_OATHS;
